...
function applyRelationship(state, result, settings) {
    ...
    // Лестница, обрывающаяся на текущей ступени, бесполезна: она показывает, где
    // история побывала, вместо того чтобы показать, куда идёт. Молча принять
    // такой ответ значит оставить раздел без следующего шага и без цели, поэтому
    // пересборка считается неудавшейся и раздел остаётся прежним.
    if (state.relationship.ladder.every(rung => rung.reached)) {
        state.relationship = previous;
        throw new Error('Модель не оставила ни одной ступени впереди — нажмите пересборку ещё раз');
    }
